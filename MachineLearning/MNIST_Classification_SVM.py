...
def load_data(folder_path, dimension=(64, 64)):
    ...
    print(categories)
    description = "Custom MNIST classification dataset"
    images=[]
    flat_data = []
    target = []
...
def predict_custom_image(image):
    model = joblib.load('./models/svm_mnistclf_model.pkl')
    img = imread(image)
...
def main():
    categories, image_dataset = load_data('./MNIST')
    ...
    joblib.dump(clf, './models/svm_mnistclf_model.pkl')
    y_pred = clf.predict(X_test)
    print("Classification report for - \n{}:\n{}\n".format(
        clf, metrics.classification_report(Y_test, y_pred)))
    for i in ('./images/1_test2.jpg','./images/0_test.jpg'):
        predict = predict_custom_image(i)
        print(i)
